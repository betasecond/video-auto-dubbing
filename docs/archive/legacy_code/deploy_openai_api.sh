#!/bin/bash
# 远程服务器 OpenAI API 部署脚本
# 在远程服务器上执行此脚本

set -e

echo "========================================="
echo "IndexTTS OpenAI Compatible API 部署脚本"
echo "========================================="
echo ""

# 1. 备份原始文件
echo "[1/5] 备份原始 api_server_v2.py ..."
...
# 2. 停止当前服务
echo "[2/5] 停止当前运行的服务 ..."
pkill -f "python api_server_v2.py" || true
sleep 2
echo "✓ 服务已停止"
echo ""

# 3. 应用补丁（替换文件）
echo "[3/5] 应用 OpenAI 兼容补丁 ..."

cat > api_server_v2.py << 'EOFPYTHON'
import os
import asyncio
import io
import traceback
...
# 检查进程
if pgrep -f "python api_server_v2.py" > /dev/null; then
    echo "✓ 服务启动成功 (PID: $(pgrep -f 'python api_server_v2.py'))"
else
    echo "✗ 服务启动失败，请检查日志: tail -f /tmp/api_server_openai.log"
    exit 1
fi
echo ""

# 5. 验证端点
echo "[5/5] 验证 OpenAI 兼容端点 ..."
sleep 2

# 检查 /v1/audio/speech 端点
curl -s -o /tmp/test_openai.wav \
  -H 'Content-Type: application/json' \
  -d '{"model":"tts-1","input":"你好，这是测试。","voice":"alloy","response_format":"wav"}' \
  http://localhost:6006/v1/audio/speech

if [ -f /tmp/test_openai.wav ] && [ $(wc -c < /tmp/test_openai.wav) -gt 1000 ]; then
    echo "✓ OpenAI API 测试成功！"
    file /tmp/test_openai.wav
    ls -lh /tmp/test_openai.wav
else
    echo "✗ OpenAI API 测试失败"
    cat /tmp/test_openai.wav
fi
echo ""
...
echo ""
echo "可用端点："
echo "  - POST /v1/audio/speech  (OpenAI 兼容)"
echo "  - GET  /v1/audio/voices   (列出可用音色)"
echo "  - POST /tts_url           (原生接口)"
echo ""
echo "日志位置："
echo "  - /tmp/api_server_openai.log"
echo "  - /root/index-tts-vllm/logs/api_server_v2.log"
echo ""
echo "外部访问地址："
echo "  - https://u861448-ej47-562de107.bjb2.seetacloud.com:8443/v1/audio/speech"
echo "========================================="
